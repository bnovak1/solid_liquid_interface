...
centro_change_data = np.array([liquid_data[0] - solid_data[0],
                             np.sqrt(liquid_data[1]**2.0 + solid_data[1]**2.0)])

centro_change_data *= natoms_per_cell/(lattice_param**3.0)

np.savetxt(outfile, centro_change_data.reshape(1, 2),
           header='Solidification centrosymmetry parameter change ' + \
                  '(angsrom^3/(angstrom^3 solid) | uncertainty for 95% CI')
